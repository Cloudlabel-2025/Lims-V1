...
import { money, hasUrl, inputStyle } from "../_components/helpers";
...
export default function ManualPage() {
  ...
  return (
    <form className="form-card" onSubmit={handleSubmit} style={{ padding: 20, borderRadius: 8, display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {form.lines.map((line, index) => (
          <div key={index} style={{ display: "grid", gridTemplateColumns: "minmax(220px, 1fr) 140px 140px 40px", gap: 10, alignItems: "center" }}>
            ...
            <input className="lims-input" type="number" min="0" max="9999999999" step="0.01" placeholder="Debit" value={line.debit} onChange={(e) => updateLine(index, { debit: e.target.value })} style={inputStyle()} />
            <input className="lims-input" type="number" min="0" max="9999999999" step="0.01" placeholder="Credit" value={line.credit} onChange={(e) => updateLine(index, { credit: e.target.value })} style={inputStyle()} />
            {form.lines.length > 2 && (
      ...
      <button className="btn-lims-primary" disabled={saving || !balanced} style={{ height: 38 }}>{saving ? "Posting..." : "Post Journal"}</button>
    </form>
  );
}
