...
test("tenant subscription experience exposes usage and notification thresholds", () => {
  ...
  const notificationRoute = fs.readFileSync(path.join(rootDir, "src/app/api/notifications/route.js"), "utf8");
  ...
  assert.match(notificationRoute, /utilizationPercent < 80/);
  assert.match(notificationRoute, /subscription-assigned/);
  assert.match(notificationRoute, /href:\s*"\/subscription"/);
